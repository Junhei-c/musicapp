...
object WidgetUpdater {
    ...
    fun updateStandard(context: Context) {
        ...
        val duration = playerManager.getDuration().takeIf { it > 0 } ?: 1L
        val progress = ((playerManager.getExoPlayer().currentPosition.toFloat() / duration) * 100).toInt()
        ...
        ids.forEach { id ->
            ...
            views.setTextViewText(R.id.widgetSongTitle, song?.name ?: "No Title")
            if (!isExpanded) {
                views.setTextViewText(R.id.widgetArtist, "Unknown Artist")
            }
            ...
            val likeIcon = if (song?.id != null && LikedSongsManager.likedSongs.contains(song.id))
            ...
            if (isExpanded) {
                ...
                views.setInt(R.id.btn_mode1, "setBackgroundResource",
                    if (selectedMode == 0) R.drawable.bg_mode_selected else R.drawable.bg_gray_rounded)
                views.setInt(R.id.btn_mode2, "setBackgroundResource",
                    if (selectedMode == 1) R.drawable.bg_mode_selected else R.drawable.bg_gray_rounded)
                views.setInt(R.id.btn_mode3, "setBackgroundResource",
                    if (selectedMode == 2) R.drawable.bg_mode_selected else R.drawable.bg_gray_rounded)
            }
    ...
    fun updateCircle(context: Context) {
        ...
        ids.forEach { id ->
            ...
            views.setImageViewResource(R.id.circle_like,
                if (song?.id != null && LikedSongsManager.likedSongs.contains(song.id)) R.drawable.redheart else R.drawable.heart)
